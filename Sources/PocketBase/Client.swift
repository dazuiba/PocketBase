...
public class Client {
    ...
    // var collections: CollectionService
...
//    var cancelControllers: [String: AbortController] = [:]
    var recordServices: [String: RecordService] = [:]
    var enableAutoCancellation: Bool = true
    ...
    func collection<M: RecordModel>(idOrName: String) -> RecordService<M> {
        if recordServices[idOrName] == nil {
            recordServices[idOrName] = RecordService(self, idOrName)
        }
        return recordServices[idOrName] as! RecordService<M>
    }

    func autoCancellation(enable: Bool) -> Client {
